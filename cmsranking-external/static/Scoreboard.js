...
var Scoreboard = new function () {
    ...
    self.init = function () {
        ...
        self.tcols_el2 = $('#Quizboard_cols');
        self.thead_el2 = $('#Quizboard_head');
        self.tbody_el2 = $('#Quizboard_body');
    ...
    self.generate = function () {
        ...
        self.tcols_el2.html(self.make_cols());
        self.thead_el2.html(self.make_head2());
        self.tbody_el2.html(self.make_body2());
    ...
    <th colspan=\"10\" class=\"f_name\">Team</th>";
        ...
        for (var i in contests) {
            ...
            for (var j in tasks) {
                ...
                result += " \
    <th colspan=\"10\" class=\"score task\" data-task=\"" + t_id + "\" data-sort_key=\"t_" + t_id + "\"><abbr title=\"" + task["name"] + "\">" + task["short_name"] + "</abbr></th>";
            }
        }
...
<tr> \
    ...
    <th colspan=\"10\" class=\"score task\" data-task=\"" + t_id + "\" data-sort_key=\"t_" + t_id + "\"><abbr title=\"" + task["name"] + "\">" + task["short_name"] + "</abbr></th>";
            ...
            result += " \
    <th colspan=\"10\" class=\"score contest\" data-contest=\"" + c_id + "\" data-sort_key=\"c_" + c_id + "\">" + contest["name"] + "</th>";
        }
        ...
        for (var u_id in DataStore.users) {
            ...
            for (var i in contests) {
                ...
                for (var j in tasks) {
                    var task = tasks[j];
                    var t_id = task["key"];
                    quiz["t_" + t_id] += user["t_" + t_id]; 
                    //quiz["c_" + c_id] += user["c_" + c_id]; 
                }
            }
        }
        var result = " \
...
<tr class=\"user\" data-user=\"" + user["key"] + "\"> \
    ...
    <td colspan=\"10\" class=\"score task " + score_class + "\" data-task=\"" + t_id + "\" data-sort_key=\"t_" + t_id + "\">" + round_to_str(user["t_" + t_id], task["score_precision"]) + "</td>";
            ...
            result += " \
    <td colspan=\"10\" class=\"score contest " + score_class + "\" data-contest=\"" + c_id + "\" data-sort_key=\"c_" + c_id + "\">" + round_to_str(user["c_" + c_id], contest["score_precision"]) + "</td>";
        }
